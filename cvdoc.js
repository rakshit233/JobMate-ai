...
const printDoc = (elementId) => {
  ...
  win.document.write(`
    ...
    <style>
      /* @page margin repeats on EVERY physical page automatically, so page 2+
         gets the same breathing room as page 1 (baked-in div padding only
         appears once, at the very top of the flowing content). */
      @page { margin: 18mm 16mm; size: A4; }
      @media print { .no-print { display: none !important; } [contenteditable] { background: transparent !important; border: none !important; } }
      html, body { margin: 0; padding: 0; height: auto !important; font-family: 'Helvetica Neue', Arial, sans-serif; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
      * { box-sizing: border-box; }
      /* The rendered document (StaticResume / CV / cover letter) carries its own
         mm padding so it looks like a page on screen. In print, @page already
         reserves that on every page — so zero the element's own padding to avoid
         doubling it on page 1, and removing overflow fixes page-break height calc. */
      body > * { padding: 0 !important; box-shadow: none !important; }
    </style></head>
