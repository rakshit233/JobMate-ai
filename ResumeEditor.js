...
export default function ResumeEditor({ profile, profiles = [], activeProfileId, onSwitchProfile, quickApplyCV, resumeVersions = [], onSaveVersion, onDeleteVersion, checkAndConsumeCredit }) {
  ...
  const [data, setData] = useState(() => {
    ...
    if (quickApplyCV) {
      // New path: structured CV handed over from CV Tailor / Quick Apply — it's
      // already in the editor's data shape, so use it directly (this is what
      // makes the tailored resume show correctly instead of generic profile data).
      if (typeof quickApplyCV === "object" && quickApplyCV.structured) {
        const s = quickApplyCV.structured;
        return {
          name: s.name || profile?.name || "Your Name",
          contact: s.contact || [profile?.email, profile?.phone, profile?.location].filter(Boolean).join(" | "),
          summary: s.summary || "",
          experience: (s.experience && s.experience.length) ? s.experience : profileToResumeData(profile).experience,
          education: (s.education && s.education.length) ? s.education : profileToResumeData(profile).education,
          skills: s.skills || profileToResumeData(profile).skills,
        };
      }
      // Legacy fallback: plain text (name on line 1, contact on line 2).
      const lines = String(quickApplyCV).split("\n");
