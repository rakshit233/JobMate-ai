          ...
          <Card style={{ marginTop: 16 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, flexWrap: "wrap", gap: 8 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                ...
                {onGoToResume && (
                  <button onClick={() => onGoToResume(result.text, profile, result.structured)}
                    style={{ padding: "6px 13px", borderRadius: 6, border: `0.5px solid ${C.gray200}`, background: C.white, fontSize: 12, cursor: "pointer", color: C.gray600, fontFamily: FONT }}>
...
export default function App() {
  ...
  const goToResumeEditor = (cvText, _profile, structured) => {
    // Prefer the structured CV (from CV Tailor / Quick Apply) — the editor
    // speaks that shape natively. Fall back to raw text for older callers.
    setQuickApplyCV(structured ? { structured } : cvText);
    setActive("resume");
  };
