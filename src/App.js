...
const JobTracker = () => {
  const [jobs, setJobs] = useState([
    ...
    { id: 3, role: "Operations Manager", company: "N26", location: "Remote", date: "2026-06-18", status: "Saved", notes: "Apply this week" },
  ]);
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        ...
        <button onClick={() => { setForm({ role:"",company:"",location:"",date:"",status:"Saved",notes:"" }); setEditId(null); setShowForm(true); }}
      ...
      <Card style={{ padding: 0, overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr style={{ background: C.gray50, borderBottom: `0.5px solid ${C.gray200}` }}>
            {["Role","Company","Location","Date","Status","Notes",""].map(h => <th key={h} style={{ padding: "9px 14px", fontSize: 11, fontWeight: 700, color: C.gray600, textAlign: "left" }}>{h}</th>)}
          </tr></thead>
          <tbody>
            {jobs.map((job, i) => (
              <tr key={job.id} style={{ borderBottom: `0.5px solid ${C.gray100}`, background: i%2===0 ? C.white : C.gray50 }}>
                ...
                <td style={{ padding:"10px 14px", fontSize:12, color:C.gray400, maxWidth:140, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{job.notes||"—"}</td>
                <td style={{ padding:"10px 14px", whiteSpace:"nowrap" }}>
                  ...
                  <button onClick={() => setJobs(jobs.filter(j => j.id!==job.id))} style={{ background:"none", border:"none", cursor:"pointer", color:C.red, fontSize:12, fontWeight:600, fontFamily:FONT }}>Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(15,31,61,0.45)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:100 }}>
          <div style={{ background:C.white, borderRadius:14, padding:24, width:"100%", maxWidth:460 }}>
            ...
            <div style={{ display:"flex", gap:10, marginTop:18, justifyContent:"flex-end" }}>
              ...
              <button onClick={save} disabled={!form.role||!form.company} style={{ padding:"8px 18px", borderRadius:8, background:C.accent, color:C.white, border:"none", fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:FONT }}>Save</button>
            </div>
          </div>
        </div>
      )}
...
const PAGES = {
  ...
  interview: { title:"Interview prep",   sub:"Get tailored questions with model answers, then practice with AI feedback." },
...
export default function App() {
  ...
  if (active === "resume") {
    return <ResumeEditor onBack={() => { setActive("apply"); setQuickApplyCV(null); }} profile={profile} quickApplyCV={quickApplyCV} />;
  }
    ...
    <div style={{ display:"flex", height:"100vh", fontFamily:FONT, background:C.gray50 }}>
        ...
        <div style={{ padding:"10px 12px", borderTop:"0.5px solid rgba(255,255,255,0.08)" }}>
          <div style={{ display:"flex", alignItems:"center", gap:9 }}>
            <div style={{ width:28, height:28, borderRadius:"50%", background:C.accent, display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:700, color:"#fff", flexShrink:0 }}>
              {profile?.name ? profile.name.split(" ").map(w=>w[0]).join("").slice(0,2).toUpperCase() : "JM"}
            </div>
            <div>
              <div style={{ fontSize:12, color:"rgba(255,255,255,0.85)", fontWeight:500 }}>{profile?.name || "Your name"}</div>
              <div style={{ fontSize:10, color:"rgba(255,255,255,0.3)" }}>Free plan · app.jobmate.tech</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"24px 28px" }}>
          ...
          {active === "tracker"   && <JobTracker />}
          {active === "apply"     && <QuickApply profile={profile} onGoToResume={goToResumeEditor} />}
          {active === "cv"        && <CVTailor profile={profile} />}
